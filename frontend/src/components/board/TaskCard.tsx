...
const getIconForTitle = async (title: string): Promise<string> => {
  ...
  const promise = fetch(`https://api.iconify.design/search?query=${keyword}&limit=1`)
    .then(res => res.json())
    .then(data => {
      const icon = data.icons?.[0] ?? 'lucide:circle-dashed';
...
export default function TaskCard({ task, index }: { task: Task; index: number }) {
  ...
  const [iconName, setIconName] = useState('lucide:circle-dashed');
  ...
  useEffect(() => {
    let mounted = true;
    getIconForTitle(task.title).then(icon => { if (mounted) setIconName(icon); });
  ...
  const isLate = task.timerStatus === 'completed' && task.actualTime / 60 > task.estimatedTime && task.estimatedTime > 0;
    ...
    <>
      <Draggable draggableId={task.id} index={index}>
        {(provided, snapshot) => (
          ...
          >
            {isRunning && <div className="absolute -inset-0.5 bg-emerald-500 rounded-lg blur opacity-30 animate-pulse" />}
            <div className={cn(
              ...
              <div className="flex items-center gap-1.5 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <span className="text-xs font-semibold text-slate-700 block truncate leading-snug">{task.title}</span>
                  {(task.description || task.actualTime > 0 || task.estimatedTime > 0 || isRunning) && (
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {task.description && <AlignLeft className="h-2.5 w-2.5 text-slate-400 shrink-0" />}
                      {(task.actualTime > 0 || task.estimatedTime > 0 || isRunning) && (
                        <span className={cn(
                          ...
                          isRunning ? 'bg-emerald-100 text-emerald-700' : isLate ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-600'
                        )}>
              ...
              <div className="flex items-center gap-0.5 shrink-0 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                <Button
                  ...
                  onClick={stopProp(() => isRunning ? endTimer(task.id) : startTimer(task.id))}
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-400 hover:text-red-500 shrink-0 rounded" onClick={stopProp(() => setIsAlertOpen(true))}>
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent onClick={e => e.stopPropagation()}>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={e => { e.stopPropagation(); setIsAlertOpen(false); }}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={e => { e.stopPropagation(); deleteTask(task.id); setIsAlertOpen(false); }} className="bg-red-500 hover:bg-red-600">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
