...
const COLUMN_ICONS: Record<string, React.ReactNode> = {
  ...
  'in-progress': <Loader className="h-4 w-4 text-amber-500" />,
  'in-review': <Clock className="h-4 w-4 text-purple-500" />,
  'done': <CheckCircle className="h-4 w-4 text-blue-600" />,
};
    ...
    <div className="w-72 sm:w-80 shrink-0 bg-slate-100/80 shadow-sm border border-slate-200 rounded-lg flex flex-col max-h-full">
      <div className="px-3 py-2.5 sm:px-4 sm:py-3 font-semibold text-slate-700 flex items-center justify-between border-b border-slate-200/50 text-sm">
        <div className="flex items-center gap-1.5">
          {COLUMN_ICONS[column.id] ?? <ListTodo className="h-4 w-4 text-slate-400" />}
      ...
      <div className="p-1.5">
        <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
          <AlertDialogTrigger className="w-full flex items-center justify-start text-slate-500 hover:bg-slate-200 hover:text-slate-800 h-8 text-xs px-2 rounded-md transition-colors">
            <Plus className="h-3.5 w-3.5 mr-1.5" />
            Add a card          </AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Add a card to {column.title}</AlertDialogTitle>
              <AlertDialogDescription>Enter a title for your new card.</AlertDialogDescription>
            </AlertDialogHeader>
            <div className="py-4">
              <Input
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={e => { e.preventDefault(); handleAddTask(); }}>Add</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
