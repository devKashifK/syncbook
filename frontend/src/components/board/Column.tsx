...
export default function ColumnComponent({ column, tasks, boardId }: ColumnProps) {
  ...
  return (
    <div className="w-80 shrink-0 bg-slate-100/80 shadow-sm border border-slate-200 rounded-xl flex flex-col max-h-full">
      <div className="px-4 py-3 font-semibold text-slate-700 flex items-center justify-between border-b border-slate-200/50">
        <div className="flex items-center gap-2">
          {getIcon()}
          <span>{column.title}</span>
        </div>
        <span className="text-xs bg-white text-slate-600 px-2.5 py-0.5 shadow-sm rounded-full border border-slate-200">
          {tasks.length}
        </span>
      </div>
      ...
      <div className="p-2">
        <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
          <AlertDialogTrigger asChild>
            <Button
              variant="ghost"
              className="w-full justify-start text-slate-500 hover:bg-slate-200 hover:text-slate-800"
            >
