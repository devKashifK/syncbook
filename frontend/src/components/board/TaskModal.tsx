...
export default function TaskModal({ task, isOpen, onClose }: TaskModalProps) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => { setTitle(t); setDescription(d); setEstimatedTime(e); }, 0);
  }, [task.title, task.description, task.estimatedTime]);

  const handleSave = () => {
    updateTask(task.id, { title, description, estimatedTime: parseInt(estimatedTime, 10) || 0 });
    onClose();
  };

  const handleToggleTimer = () => {
    updateTask(task.id, { estimatedTime: parseInt(estimatedTime, 10) || 0 });
    task.timerStatus === 'running' ? endTimer(task.id) : startTimer(task.id);
  };
    ...
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="sm:max-w-[420px] bg-white p-0 overflow-hidden shadow-lg border border-slate-200 rounded-lg">
        <div className="px-5 pt-4 pb-3 border-b border-slate-100">
          <DialogHeader>
            <div className="flex items-center gap-1.5 text-slate-400">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-wider">Task details</span>
            </div>
            <div className="mt-1">
              <Input
                value={title}
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Estimate (m)</label>
                <Input
                  type="number"
                  value={estimatedTime}
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Actual Time</label>
                <div className="h-8 flex items-center justify-between bg-slate-50 border border-slate-200 rounded-md px-2 text-xs font-mono text-slate-700 w-full">
                  <span>{formatTimeMs(task.actualTime)}</span>
                  <Button
                    size="sm" variant="ghost"
                    onClick={handleToggleTimer}
                    className={cn(
                      'h-6 px-1.5 text-[9px] font-bold flex items-center gap-1 rounded',
                      isRunning ? 'text-red-600 hover:bg-red-50' : 'text-blue-600 hover:bg-blue-50'
                    )}
                  >
                    {isRunning
        ...
        <div className="px-5 py-3 bg-slate-50 flex justify-end gap-2 border-t border-slate-200">
          ...
          <Button size="sm" onClick={handleSave} className="h-8 text-xs bg-blue-600 hover:bg-blue-700 text-white shadow-none font-semibold px-4">Save</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
